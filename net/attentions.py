...
class MultiHeadDotProduct(nn.Module):
    ...
    def forward(self, feats: torch.tensor, edge_index: torch.tensor,
        ...
        q = k = v = feats#（10,128）嵌入矩阵
        bs = q.size(0)

        # FC layer and split into heads --> h * bs * embed_dim
        k = self.k_linear(k).view(bs, self.nhead, self.hdim).transpose(0, 1)#2,10,64
        ...
        feats = self._attention(q, k, v, edge_index, edge_attr, bs)#多头注意力机制2,10,64
        # concatenate heads and put through final linear layer
        feats = feats.transpose(0, 1).contiguous().view(
            bs, self.nhead * self.hdim)
        feats = self.out(feats)#10,128
    ...
    def _attention(self, q, k, v, edge_index=None, edge_attr=None, bs=None):
        ...
        a = q.index_select(1, c)
        a = a.unsqueeze(dim=-2)
        b = k.index_select(1, r).unsqueeze(dim=-1)
        scores = torch.matmul(
            q.index_select(1, c).unsqueeze(dim=-2),
            k.index_select(1, r).unsqueeze(dim=-1))
        scores = scores.view(self.nhead, e, 1) / math.sqrt(self.hdim)#2,120,1
        ...
        out = self.aggr(out, c, 1, bs)  # H x bs x hdim   这里默认是add在attention文件
        if type(out) == tuple:
            out = out[0]
        return out
