...
class MetaLayer(torch.nn.Module):
    ...
    def forward(self, feats, edge_index, edge_attr=None):
        ...
        if self.node_model is not None:#节点模型
...
class GNNReID(nn.Module):
    ...
    def forward(self, feats, edge_index, edge_attr=None, output_option='norm'):
        r, c = edge_index[:, 0], edge_index[:, 1]#边的关系索引
        
        if self.dim_red is not None:#经过一层线性层
...
class GNNNetwork(nn.Module):
    ...
    def forward(self, feats, edge_index, edge_attr):
        ...
        for layer in self.layers:
            feats, egde_index, edge_attr = layer(feats, edge_index, edge_attr)#点积注意力机制，层数
            out.append(feats)
...
class DotAttentionLayer(nn.Module):
    ...
    def forward(self, feats, egde_index, edge_attr):
        ...
        # feats = feats + feats2 if self.res1 else feats2#情况1
        feats = feats2 #情况2
