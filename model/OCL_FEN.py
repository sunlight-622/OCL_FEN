...
from .losses import BatchCriterion, statm_loss,cos_loss
...
class Net(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # setup network
        self.is_cifar = any(x in str(args.data_file) for x in ['cifar', 'cub', 'mini', 'svhn'])
        if 'cifar' in args.data_file or 'mini' in args.data_file or 'svhn' in args.data_file:
            self.net = ResNet18(n_outputs)
        elif 'cub' in args.data_file:
            self.net = ResNet18Full(True, n_outputs)
        else:
            #self.net = MLP([n_inputs] + [nh] * nl + [n_outputs])
            self.net = MLP([784] + [128] * 3 )
        ...
        self.beta= args.beta
        self.con_kl = args.con_kl
        self.con_klkl = args.con_klkl
        self.cos = args.cos
        self.kl = args.kl
        ...
        self.criterion = BatchCriterion(1.0, self.Con_T) 
        ...
        self.n_val = int(self.n_memories * 0.2)#确认验证集和记忆集的数量0.2/0.8
        ...
        if 'cub' in args.data_file:
            ...
            self.memx = torch.FloatTensor(n_tasks, self.n_memories, 3, 64, 64)#？？
            self.valx = torch.FloatTensor(n_tasks, self.n_val , 3, 64, 64)
        else:
        ...
        self.valy = torch.LongTensor(n_tasks, self.n_val)
                #建立特征记忆
        if 'cub' in args.data_file:
            self.mem_feat_layer = torch.FloatTensor(n_tasks, self.n_memories, 512).fill_(0)
        else:
            self.mem_feat_layer = torch.FloatTensor(n_tasks, self.n_memories, 160).fill_(0)
        ...
        if args.cuda:
            ...
            self.mem_feat_layer = self.mem_feat_layer.cuda()
    ...
    def memory_sampling(self,t, valid = False):
        ...
        if valid:
            ...
            mem_feat_layer = self.mem_feat_layer[:t,:]
        ...
        feat_layer = mem_feat_layer[t_idx, s_idx]
        ...
        if valid:
            ...
            return xx,yy, feat , mask, t_idx.tolist(), feat_layer
    def observe(self, x, t, y):
        if t != self.current_task:
            tt = self.current_task
            offset1, offset2 = self.compute_offsets(tt)
            out,  feat_layer= self.forward(self.memx[tt],tt, True)
            self.mem_feat[tt] = F.softmax(out[:, offset1:offset2] / self.temp, dim=1 ).data.clone()

            self.mem_feat_layer[tt] = feat_layer.data.clone()
            self.current_task = t
            self.mem_cnt = 0
            self.val_cnt = 0
            self.val_count = 0
        ...
        for _ in range(self.n_meta):
            ...
            for i in range(self.inner_steps):
                ...
                if t > 0:
                    xx, yy, feat, mask, list_t, feat_layer = self.memory_sampling(t)
                    pred_ , cur_feat = self.net(xx)

                    # a = 1-torch.cosine_similarity(feat_layer, cur_feat, dim=1)
                    #保存内部特征改变
                    # if self.b is None:
                    #     self.b = a
                    # else:
                    #     self.b = torch.cat((self.b,a),dim=0)
                    # if t == 2:
                    #     torch.save(self.b, 'tiny_fature_1.pt')

                    pred = torch.gather(pred_, 1, mask)
                    loss2 = self.bce(pred, yy)
                    loss3 = self.reg * self.kl(F.log_softmax(pred / self.temp, dim = 1), feat)
                    loss5 = self.lamda*(1-torch.cosine_similarity(feat_layer, cur_feat, dim=1))
                    loss6 = self.lamda*F.mse_loss(feat_layer, cur_feat)
                    con_feat = torch.cat((cur_feat, feat_layer),0)
                    loss4 = self.lamda*self.criterion(con_feat)
                    #lamda = np.exp(-2*((1.0*t)/20.))
                    # lamda = (1/8)*torch.sqrt(torch.tensor(t/20.)).cuda()
                    
                    if self.con_klkl:
                        
                        loss = loss1 + loss2 + loss4 + loss3
                    if self.con_kl:
                        loss = loss1 + loss2 + loss4 
                        # loss = loss1 + loss2 + loss5 
                        # loss = loss1 + loss2 + loss6

                    if self.cos:
                        loss5 =self.lamda* self.cos_loss(cur_feat, feat_layer)
                        loss = loss1 + loss2 + loss5                        
